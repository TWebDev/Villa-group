...
$.fn.vgSign = function (input) {
    return this.each(function () {
        var id = this.id;
        var msg = "";
        if (window.location.pathname !== "/membership/Preview") {
             msg = 'By checking this box I understand and agree with the disclaimer shown above and i accept the charge of 50 dlls to my credit card.';
        } else {
            msg = 'By checking this box I understand and agree with the disclaimer shown above ';
        }
        ...
        var htmlElements = '<div>'
            + '<input type="checkbox" id="chkAccept" onchange="Sign.init();" /><label for="chkAccept">' + msg + '</label></div>'

            + '<div class="alert alert-warning mt-3" id="spnPleaseAccept">Please accept using the check box above.</div>'
        + '<span id="vgsignature_clear" onclick="clear()" style="border: 1px solid #E3E3E3;border-radius: 2px 2px 2px 2px;color: #0D7DE4;cursor: pointer; padding: 7px;position: absolute; margin-top: 37px; z-index: 2;">Clear</span>'
      ...
      //  + '<input type="button" class="btn btn-primary" id="vgsignature_save" onclick="Sign.sign(\'' + transactionID + '\');" value="Send" />'
        ...
        $('#vgsignature_clear').click(Sign.clear);
...
var Sign = function () {
    ...
    function saveSignature(leadid,type) {
            ...
            if (leadid !== 1) {
             ...
             //   $('#vgsignature_title').html('Saving your signature...');
              ...
              //  $('#vgsignature_clear').hide();
                ...
                $.ajax({
                    url: '/Content/management/saveSignature',
                    cache: false,
                    type: 'POST',
                    data: { leadid: leadid, signature: myImage, type: type },
                    success: function (data) {
                        if (data) {
                            clear();
